...
#     pinyin = formatted_pinyin(line.split(/[\[\]]/)[1])
#     entry = Entry.find_by(traditional: traditional, 
#                           simplified: simplified, 
#                           pinyin: pinyin) || 
#             Entry.create!(traditional: traditional, 
#                           simplified: simplified, 
#                           pinyin: pinyin,
#                           freq_rank: -1)
#     freq_query = "#{traditional} #{simplified} \\[#{pinyin.gsub(/\s+/,"")}\\]"
#     freq_rank = `awk '/#{freq_query}/ {print FNR}' chinese_words_frequency_list_pruned`.strip
#     if not freq_rank.empty?
#         entry.update(freq_rank: freq_rank)
#     end
#     defs = line.split("/")[1..-2]
#     defs.each { |d| 
...
File.foreach(dictionary) { |line| 
    ...
    entry = Entry.find_by(traditional: traditional, 
            ...
            Entry.create!(traditional: traditional, 
                          ...
                          pinyin: pinyin,
                          freq_count: 0)
    freq_query = "#{traditional} #{pinyin}"
    freq_count = `awk '/#{freq_query}/ {printf("%s", $3)}' character_syllable_freq_pruned`
    if not freq_count.empty?
        entry.update(freq_count: freq_count)
    end
    defs = line.split("/")[1..-2]
    defs.each { |d| 
        entry.definitions.create!(definition: d, count: 0)
